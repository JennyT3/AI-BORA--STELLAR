...
import { Check, Bot, ExternalLink } from 'lucide-react';
import { useSectionInView } from '../hooks/useSectionInView';
import { createContact } from '../services/firebase';
import { sendMensagemConfirmation } from '../services/emailService';
...
export function CTAFooterSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.email) {
        try {
          await sendMensagemConfirmation(formData.email, formData.name);
        } catch (emailErr) {
      ...
      setFormData({ name: '', nameNegocio: '', phone: '', email: '', message: '' });
  ...
  return (
    <section id="contact" style={{ backgroundColor: '#F8F7F4', padding: '80px 16px' }}>
