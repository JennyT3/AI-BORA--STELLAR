...
import { TasksKanban } from "./admin/TasksKanban";
import { useAuth } from "../hooks/useAuth";
import { useAdminData } from "../hooks/useAdminData";
import type { ClientStage } from "../types";
import { ClientFormModal } from "../components/admin/ClientFormModal";
import { InvoiceModal } from "../components/admin/InvoiceModal";
import { Bot, Home, GraduationCap, ExternalLink, MessageCircle, X, Send } from "lucide-react";

type AdminTab = "dashboard" | "proposals" | "clients" | "tasks";
  ...
  const FlowCard = () => (
    <div style={{ 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
        {[
          { step: '1', title: 'Create Proposal', desc: 'Admin creates proposal → PDF with Stellar hash', link: '/admin/quote' },
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F8F7F4", display: "flex", position: "relative" }}>
      <Sidebar
        ...
        clientCount={admin.clients.length}
        ...
        <main style={{ 
          ...
          {activeTab === "dashboard" && (
            <Dashboard
              ...
              onExport={() => {}}
              onNewQuote={() => navigate("/admin/quote")}
              onNewClient={() => { admin.setClientFormData({ name: "", email: "", mobile: "", taxId: "", address: "", category: "curious", notes: "" }); admin.setShowClientForm(true); }}
              onNewInvoice={() => admin.openInvoice({} as any)}
          ...
          {activeTab === "proposals" && (
            <Proposals
              ...
              onMarkSent={admin.handleMarkSent}
              onRegistrarResponse={admin.handleRegistrarResponse}
              onRefresh={admin.loadProposals}
              onEditQuote={id => navigate(`/admin/quote?edit=${id}`)}
            />
          )}
          ...
          {activeTab === "clients" && (
            <Clients
              clients={admin.clients}
              salesReps={admin.salesReps}
              search={admin.clientSearch}
              onSearchChange={admin.setClientSearch}
              filterCategory={admin.clientFilterCategory}
              onFilterCategoryChange={admin.setClientFilterCategory}
              filterSource={admin.clientFilterSource}
              onFilterSourceChange={admin.setClientFilterSource}
              filterResponse={admin.clientFilterResponse}
              onFilterResponseChange={admin.setClientFilterResponse}
              sortBy={admin.clientSortBy}
              onSortByChange={admin.setClientSortBy}
              sortOrder={admin.clientSortOrder}
              onSortOrderChange={admin.setClientSortOrder}
              selectedClient={admin.selectedClient}
              onSelectClient={admin.setSelectedClient}
              onNewClient={() => { admin.setClientFormData({ name: "", email: "", mobile: "", taxId: "", address: "", category: "curious", notes: "" }); admin.setShowClientForm(true); }}
              onLinkProposal={admin.handleLinkProposal}
              onViewProposal={id => window.open(`/p/${id}`, "_blank")}
              onVerRecord={id => { const c = admin.clients.find(x => x.id === id); if (c) admin.setSelectedClient(c); }}
              onEditProposal={id => { admin.handleEdit(id); setActiveTab("proposals"); }}
              onInvoice={c => admin.openInvoice(c)}
              onEdit={(c) => { admin.setClientFormData(c); admin.setShowClientForm(true); }}
              onDelete={admin.handleDeleteClient}
              onNavigateInvoicing={() => {}}
              onUpdateStage={async (id, stage) => { 
                try {
                  const { updateClient } = await import('../services/firebase');
                  await updateClient(id, { stage: stage as ClientStage });
                  admin.loadClients();
              ...
              onUpdateTasks={async (id, tasks) => {
                try {
                  const { updateClient } = await import('../services/firebase');
                  await updateClient(id, { tasks });
                  admin.loadClients();
                } catch(e) { console.error(e); }
              }}
              onDelegateSalesRep={admin.handleDelegateSalesRep}
            />
          )}
          ...
          {activeTab === "tasks" && (
            <TasksKanban
              ...
              salesReps={admin.salesReps}
              isAdmin={true}
              onRefresh={admin.loadTasks}
              onVerRecord={(id) => { const c = admin.clients.find(x => x.id === id); if (c) admin.setSelectedClient(c); setActiveTab('clients'); }}
            />
          )}
        </main>
      </div>
      ...
      {admin.showClientForm && admin.clientFormData && (
        <ClientFormModal
          clientFormData={admin.clientFormData}
          clients={admin.clients}
          clientSearch={admin.clientSearch}
          showClientSuggestions={admin.showClientSuggestions}
          onClientSearchChange={admin.setClientSearch}
          onSuggestionSelect={(c) => { admin.setClientFormData({...admin.clientFormData, ...c}); admin.setShowClientSuggestions(false); }}
          onFieldChange={(f, v) => admin.setClientFormData({...admin.clientFormData, [f]: v})}
          onSave={admin.handleSaveClient}
          onClose={admin.closeClientForm}
        />
      )}

      {admin.showInvoiceModal && admin.invoiceDate && (
        <InvoiceModal
          invoiceDate={admin.invoiceDate}
          invoiceNumber={admin.invoiceNumber}
          onInvoiceNumberChange={admin.setNumberInvoice}
          onConfirm={() => { alert('Invoice generated'); admin.closeInvoiceModal(); }}
          onClose={admin.closeInvoiceModal}
        />
      )}
    </div>
  );
