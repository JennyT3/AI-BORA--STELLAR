...
interface ProposalsProps {
  ...
  onMarkSent: (p: any) => void;
  onRegistrarResponse: (p: any) => void;
  onRefresh: () => void;
  onEditQuote: (id: string) => void;
}

export function Proposals({ proposals, loading, editingId, editData, onSave, onCancel, onUpdateEditData, onDelete, onMarkSent, onRegistrarResponse, onRefresh, onEditQuote }: ProposalsProps) {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: 12 }}>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => onUpdateEditData({ ...editData, client: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => onUpdateEditData({ ...editData, amount: parseFloat(e.target.value) })}
                      style={{ width: '100%', padding: '12px', borderRadius: 8, border: `1px solid ${theme.colors.border}`, fontSize: 13, backgroundColor: '#fafafa', color: theme.colors.text.primary, outline: 'none' }}
                    />
                  </div>
                  <div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: isMobile ? 12 : 24, flexDirection: isMobile ? 'column' : 'row' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 13, color: theme.colors.text.secondary, marginBottom: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      <span>{p.quoteNumber}</span>
                      <span style={{ color: '#F25C05', fontWeight: 700 }}>{p.amount?.toFixed(2)} €</span>
                      <span>{p.marcas} brand{p.marcas !== 1 ? "s" : ""}</span>
                      {p.sentAt && <span style={{ color: '#3498DB' }}>Sent: {p.sentAt}</span>}
                    </div>

                    {/* Service tags */}
                    {p.services && p.services.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {p.services.slice(0, 4).map((s: string, i: number) => (
                        ...
                        {p.services.length > 4 && (
                          <span style={{ fontSize: 11, color: theme.colors.text.secondary }}>+{p.services.length - 4}</span>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: isMobile ? 12 : 0 }}>
                    ...
                    <button
                      ...
                      style={{ padding: '12px 16px', borderRadius: 8, backgroundColor: p.sentAt ? '#dbeafe' : '#f3f4f6', color: p.sentAt ? '#3498DB' : theme.colors.text.secondary, border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', minHeight: 44, display: 'flex', alignItems: 'center' }}
                    >
                      Sent
                    </button>
