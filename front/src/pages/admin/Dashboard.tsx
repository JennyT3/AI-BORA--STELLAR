import { FileText, Users, Check, TrendingUp, Download } from "lucide-react";
...
interface DashboardProps {
  stats: { total: number; sent: number; respondidas: number; aceitas: number; reagendadas: number };
  proposals: any[];
  requests: any[];
  clients: any[];
  onExport: () => void;
...
export function Dashboard({ stats, proposals, requests, clients, onExport, onNewQuote, onNewClient, onNavigate, isMobile = false }: DashboardProps) {
  const clientsActive = clients.filter(c => c.category === "client").length;
  ...
  return (
    <div style={{ fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <div style={{ 
        ...
        <NewStatsCard 
          label="Clients" 
          value={clientsActive.toLocaleString()} 
          percentage={14.2}
          percentageColor="green"
          icon={<Users size={20} />}
          iconColor="#F22283"
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 32 }}>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 24, padding: 32, boxShadow: "0px 20px 40px rgba(90, 65, 55, 0.04)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
            ...
            <button onClick={() => onNavigate("requests")} style={{ fontSize: 12, color: "#F25C05", background: "none", border: "none", cursor: "pointer", fontWeight: 800 }}>View all</button>
          </div>
            ...
            {requests.slice(0, 3).map(s => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, color: "#1b1c1b", fontSize: 14 }}>{s.name}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>{s.company || "E-commerce Integration"}</div>
                  </div>
                </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 24, padding: 32, boxShadow: "0px 20px 40px rgba(90, 65, 55, 0.04)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
            ...
            <button onClick={() => onNavigate("proposals")} style={{ fontSize: 12, color: "#F25C05", background: "none", border: "none", cursor: "pointer", fontWeight: 800 }}>View all</button>
          </div>
          <div style={{ display: "grid", gap: 12 }}>
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 800, color: "#1b1c1b", fontSize: 14 }}>{p.quoteNumber}</div>
                      <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>{p.client}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 900, color: "#1b1c1b", fontSize: 14 }}>{p.amount?.toLocaleString()} USDC</div>
