...
import { getProposal, isProposalValid, updateProposal, updateClient, createTask, getProposalByToken } from "../services/firebase";
...
export function ProposalPage() {
  ...
  const [responseSent, setResponseSent] = useState<"yes" | "not" | "reschedule" | null>(null);
  ...
  useEffect(() => {
    async function loadProposal() {
      try {
        ...
        if (!data) {
          setError("Proposal not found");
        } else if (!data.accessToken && !data.clientId) {
  ...
  const handleResponse = async (type: "yes" | "not" | "reschedule") => {
    if (!proposal || respondendo) return;
    
    if (type === 'yes' && !confirm("Do you accept this proposal?")) return;
    
    setRespondendo(true);
    
    try {
      await updateProposal(proposalId || params.id, {
        response: type === 'reschedule' ? 'reschedule' : type,
        responseDate: new Date().toISOString(),
        status: type === 'yes' ? 'accepted' : (type === 'not' ? 'declined' : 'pending')
      });

      if (proposal.email) {
        const emailResponse = type === 'reschedule' ? 'Requisitou ajustes' : (type === 'yes' ? 'Aceitou' : 'Recusou');
        sendEmail(proposal.email, 'proposal-response-confirmation', {
          clientName: proposal.client,
          response: emailResponse,
          recordUrl: type === 'yes' ? `${window.location.origin}/c/${proposal.clientId}` : '',
        }).catch(() => {});
        
        if (type === 'yes') {
          sendEmail(proposal.email, 'client-welcome', {
            clientName: proposal.client,
            recordLink: `${window.location.origin}/c/${proposal.clientId}`,
          }).catch(() => {});
        }
      }

      if (type === "yes" && proposal.clientId) {
        await updateClient(proposal.clientId, {
          category: "client",
          stage: "start",
          responseDate: new Date().toISOString(),
          response: "yes",
          recordUrl: `${window.location.origin}/c/${proposal.clientId}`
        });
        ...
        if (proposal.services && proposal.services.length > 0) {
          for (const service of proposal.services) {
            ...
            const SERVICE_DETAILS: Record<string, string[]> = {
              "Social media management": ["Monthly content planning", "Publication scheduling", "Metric reports", "Growth strategy", "Daily support and engagement"],
              "Content creation": ["Post copy", "Creative captions", "Persuasive copywriting", "Blog content", "Newsletters"],
              "Post design": ["Custom graphic posts", "Informative carousels", "Stories and Reels", "Banners and covers", "Brand templates"],
              "Community Management": ["Comment replies", "Direct messages", "Moderation", "Review management", "Engagement reports"],
              "Logo design": ["3 initial concepts", "Unlimited revisions", "Multiple formats", "Basic brand manual", "Commercial use"],
              "Corporate identity": ["Logo + variations", "Colour palette", "Typography", "Business card", "Corporate folder"],
              "Video production": ["Script and storyboard", "Professional filming", "Editing and post-production", "Motion graphics", "4K delivery"],
              "Reels creation": ["Creative concept", "Short script", "Dynamic editing", "Trending audio", "Reach optimisation"],
              "Website / landing page": ["Responsive design", "SEO optimisation", "Contact forms", "Analytics integration", "SSL included"],
              "Online store": ["Product management", "Secure payments", "Integrated shipping", "Admin panel", "Professional template"],
              ...
              "WhatsApp chatbot": ["Automatic replies", "Interactive menu", "Meeting booking", "CRM integration", "AI chatbot"],
              "AI & Automation": ["Task automation", "App integration", "Custom workflows", "Automatic reports", "Virtual assistant"],
              "Professional photography": ["2h session", "20 edited photos", "Commercial use", "Multiple setups", "High resolution"],
              "Strategic Consulting": ["Business analysis", "Digital strategy", "Action plan", "Monthly meetings", "Priority support"],
            };
            const details = SERVICE_DETAILS[serviceName] || [];
            await createTask({
              title: serviceName,
              serviceName,
              description: details.length > 0 ? "What's included:\n• " + details.join("\n• ") : '',
              clientId: proposal.clientId,
              clientName: proposal.client,
              clientEmail: proposal.email || '',
              status: "available",
              periodicidade: "one_time",
            });
          }
        }
      }

      if (type === "reschedule" && proposal.clientId) {
        await updateClient(proposal.clientId, {
          response: "reschedule",
          responseDate: new Date().toISOString(),
        });
        ...
        if (proposal.email) {
          sendEmail(proposal.email, 'proposal-rejected', {
            clientName: proposal.client,
          }).catch(() => {});
        }
        
        if (proposal.salesRepId) {
          const { getSalesRep } = await import('../services/salesReps');
          if (getSalesRep) {
            const salesRep = await getSalesRep(proposal.salesRepId);
            if (salesRep?.email) {
              sendEmail(salesRep.email, 'change-request', {
                clientName: proposal.client,
                company: proposal.company,
                proposalId: params.id,
                proposalLink: `${window.location.origin}/admin/quote?edit=${params.id}`,
              }).catch(() => {});
            }
          }
        }
  ...
  useEffect(() => {
    if (proposal?.client) {
      document.title = `Proposal for ${proposal.client} — AI BORA`;
    }
  }, [proposal]);
  ...
  if (responseSent) {
    const mensagens = {
      yes: {
        title: "Great! Proposal confirmed.",
        subtitle: "Welcome to the AI BORA family! We will be in touch within the next few hours.",
        color: "#10B981",
        emoji: "🎉"
      },
      reschedule: {
        title: "No problem!",
        subtitle: "We will review the proposal and adjust it to your needs. We will be in touch shortly.",
        color: "#F25C05",
        emoji: "↩️"
      },
      not: {
        title: "Thanks for your time.",
        subtitle: "If you change your mind, we will be here. Good luck with your business!",
        color: "#6B7280",
        emoji: "👋"
      }
    };
    
    const msg = mensagens[responseSent];
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'Montserrat, sans-serif' }}>
        <div style={{ textAlign: 'center', maxWidth: 480 }}>
          ...
          <div style={{ width: 60, height: 4, backgroundColor: msg.color, borderRadius: 2, margin: '0 auto 24px' }} />
      ...
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: '60px 24px' }}>
        ...
        <section style={{ textAlign: 'center', marginBottom: 80 }}>
          ...
          <h1 style={{ fontSize: 'clamp(32px, 5vw, 56px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 24, letterSpacing: -2, color: '#ffffff' }}>
            Hi, {proposal.client}.<br />
            Ready to grow your business?
          </h1>
          <p style={{ fontSize: 18, color: '#cccccc', maxWidth: 600, margin: '0 auto', lineHeight: 1.6 }}>
            We reviewed your profile and prepared a strategy focused on real results for <span style={{ color: '#fff', fontWeight: 700 }}>{proposal.company || 'your business'}</span>.
          </p>
        </section>
            ...
            {proposal.services?.map((service: any, idx: number) => {
              ...
              return (
                <div key={idx} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 32, transition: 'transform 0.3s' }}>
                  ...
                  <h3 style={{ fontSize: 20, fontWeight: 800, marginBottom: 12, color: '#ffffff' }}>{name}</h3>
        ...
        <section style={{ background: '#fff', color: '#1A1A1A', borderRadius: 40, padding: '80px 40px', marginBottom: 100, textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
            {[
              { icon: "📱", title: "Digital marketing", desc: "Full management of your social channels with quality content and a clear strategy.", tags: ["Instagram", "Facebook", "LinkedIn"], highlight: proposal?.services?.some((s: string) => s.includes("Social") || s.includes("Marketing") || s.includes("Management")) },
              { icon: "📊", title: "Consulting", desc: "Personalised digital strategy to help your business grow.", tags: ["Analysis", "Planning", "Reports"], highlight: proposal?.services?.some((s: string) => s.includes("Consulting") || s.includes("Analysis") || s.includes("Strategy")) },
              { icon: "🎨", title: "Design & identity", desc: "Professional visual identity that represents your brand with credibility.", tags: ["Logo", "Posts", "Banners"], highlight: proposal?.services?.some((s: string) => s.includes('Design') || s.includes("Logo") || s.includes("Identity")) },
              { icon: "💻", title: "Website", desc: "A modern, fast site optimised for search and conversions.", tags: ["Landing Page", "SEO", "Mobile"], highlight: proposal?.services?.some((s: string) => s.includes("Web") || s.includes("Landing") || s.includes("Website")) },
              { icon: "📢", title: "Paid ads", desc: "Paid campaigns managed by specialists to maximise your investment.", tags: ["Meta Ads"], highlight: proposal?.services?.some((s: string) => s.includes('Ads') || s.includes("Advertising")) },
              { icon: "⚡", title: "Automation & AI", desc: "Automate repetitive work and respond to customers 24/7 with AI.", tags: ["Chatbot", "WhatsApp Auto", "CRM"], highlight: proposal?.services?.some((s: string) => s.includes("Chatbot") || s.includes("Automation") || s.includes("AI")) },
            ].map((service, index) => (
              <div key={index} style={{ 
                background: service.highlight ? 'rgba(242,92,5,0.1)' : 'rgba(255,255,255,0.04)', 
                border: service.highlight ? '2px solid #F25C05' : '1px solid rgba(255,255,255,0.08)', 
                borderRadius: 20, 
                padding: 28, 
                transition: 'all 0.3s',
                textAlign: 'left'
                ...
                <h3 style={{ fontSize: 18, fontWeight: 800, marginBottom: 8, color: '#1A1A1A' }}>{service.title}</h3>
                <p style={{ fontSize: 14, color: '#666', lineHeight: 1.5, marginBottom: 16 }}>{service.desc}</p>
        ...
        <section style={{ textAlign: 'center', marginBottom: 100 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #F25C05 0%, #F22283 100%)', borderRadius: 32, padding: '60px 40px', display: 'inline-block', minWidth: 320 }}>
            ...
            <div style={{ fontSize: 'clamp(40px, 8vw, 64px)', fontWeight: 900, marginBottom: 8, letterSpacing: -2 }}>
              {Number(proposal.amount || proposal.total || 0).toLocaleString('en-GB', { minimumFractionDigits: 2 })}€
            </div>
        ...
        <section style={{ textAlign: 'center', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: isMobile ? 24 : 40, padding: isMobile ? '40px 20px' : '80px 40px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'center', gap: 16 }}>
            ...
            <button 
              onClick={() => { if (confirm('Are you sure you are not interested?')) handleResponse('not'); }}
              disabled={respondendo}
