...
export default function ClientLoginPage() {
  const [match, params] = useRoute('/client/login/:token');
  ...
  const handleTokenLogin = async (token: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/client-login-validate`, {
        method: 'POST',
      ...
      if (result?.email) localStorage.setItem('aibora_client_email', result.email);
      setLocation(`/c/${result.clientId}?logged=true`);
    } catch (err) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/client-magic-link`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
