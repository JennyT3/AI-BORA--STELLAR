...
import { getPaymentLink, markAsPaid } from '../services/payments';
import { getInvoice } from '../services/invoices';
// The on-chain 70/30 split requires the admin secret and is executed
// server-side by /api/stellar-pay via the PaymentSplitter contract.
import { processPaidCollaborators } from '../services/paymentToTasks';
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    if (id === 'test' || id === 'demo') {
      setPaymentLink({ id: 'test', invoiceId: 'test-invoice', clientId: 'test-client' });
      setInvoice({
        number: 'TEST-2026-001',
        description: 'AI BORA Demo Invoice',
        dueDate: '15/04/2026',
        totalAmount: 1.00,
    ...
    try {
      ...
      if (!link) {
        if (id === 'test' || id === 'demo') {
          ...
          setInvoice({
            number: 'TEST-2026-001',
            description: 'Demo Invoice - No payment required',
            dueDate: '15/04/2026',
            totalAmount: 1.00,
            clientId: 'test-client'
          });
      ...
      setPaymentLink(link);
      const invoiceDate = await getInvoice(link.invoiceId);
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/stellar-pay`, {
        ...
        body: JSON.stringify({
          ...
          amountEur: invoice.totalAmount || 1,
          memo: `AIBORA invoice ${invoice.number}`,
        }),
      });
      ...
      if (data.split?.txHash) {
        setSplitTxHash(data.split.txHash);
      } else if (data.splitError) {
        console.warn('[Pagamento] Split not executed:', data.splitError);
      }

      // Mark as paid in Firestore
      await markAsPaid(paymentLink.id);
      
      // Step 3: Mark tasks as paid and register commissions
      if (paymentLink.clientId) {
        try {
          await processPaidCollaborators(
      ...
      <div style={styles.card}>
        ...
        <p style={{ color: '#F25C05', fontWeight: 700, marginBottom: 24 }}>
          {splitTxHash
            ...
            : 'Payment recorded. On-chain split is pending configuration.'}
        </p>
        ...
        <button
          onClick={() => setLocation(`/c/${paymentLink?.clientId}`)}
      ...
      <div style={styles.card}>
        ...
        {invoice && (
          <div style={{ backgroundColor: '#f8f7f4', borderRadius: 12, padding: 20, marginBottom: 24 }}>
            <Row label="Invoice no." value={invoice.number} />
            <Row label="Description" value={invoice.description || 'AI BORA services'} />
            <Row label="Due date" value={invoice.dueDate || '15 days'} />
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12, borderTop: '1px solid #e5e7eb', marginTop: 8 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: 24, fontWeight: 900, color: '#F25C05' }}>
                  {invoice.totalAmount?.toFixed(2) || '0.00'} USDC
                </span>
                <div style={{ fontSize: 11, color: '#9ca3af' }}>
                  ≈ {invoice.totalAmount?.toFixed(2) || '0.00'} USDC (testnet)
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 16, paddingTop: 16, borderTop: '1px solid #e5e7eb' }}>
          <a href={`/c/${paymentLink?.clientId}`} style={{ color: '#666', fontSize: 13 }}>
            Back to my area
          </a>
        </div>
      </div>
