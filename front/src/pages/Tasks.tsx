...
export default function TasksPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const q = query(collection(db, 'tasks'), where('proposalId', '==', proposalId));
  ...
  const handleAssignMe = async (taskId: string) => {
    try {
      ...
      await updateTask(taskId, {
        status: 'assigned',
        assignedName: colabName,
        dataAssignment: new Date().toISOString()
      });

      setTasks(prev => prev.map(t => 
        t.id === taskId ? { ...t, status: 'assigned', assignedName: colabName } : t
      ));
  ...
  const handleSendPaymentLink = () => {
    setCopying(true);
    const paymentLink = `${window.location.origin}/payment/${proposalId}`;
      ...
      <div style={styles.header}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <div>
            ...
            <p style={styles.subtitle}>Proposal: {proposal?.quoteNumber || proposalId} - Client: {proposal?.client}</p>
          </div>
          <button 
            ...
            <div key={task.id} style={styles.taskCard}>
              ...
              <div style={styles.taskInfo}>
                ...
                <div style={{ backgroundColor: 'rgba(34, 197, 94, 0.05)', padding: '12px 20px', borderRadius: 12, border: '1px dashed #22c55e' }}>
                  ...
                  <p style={{ ...styles.taskAmount, color: '#22c55e' }}>${((task.clientAmount || 0) * 0.3).toFixed(2)}</p>
                </div>
              </div>
              ...
              {task.status === 'available' ? (
                ...
                <div style={styles.assignedInfo}>
                  Assigned to: <strong>{task.assignedName || 'Collaborator'}</strong>
                </div>
              )}
            </div>
          ))
