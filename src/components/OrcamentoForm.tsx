      ...
      <div style={{ flex: isPublicView ? "1 1 600px" : 1 }}>
        <div style={{ marginBottom: 24, backgroundColor: bgColor, borderRadius: 16, padding: 24, border: cardBorder }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isPublicView ? "1fr 1fr" : "repeat(3, 1fr)", gap: 10 }}>
            ...
            <div><label style={{ fontSize: 10, fontWeight: 600, color: theme.colors.text.secondary, display: "block", marginBottom: 4 }}>Morada</label><input value={cliente.morada} onChange={(e) => setCliente({...cliente, morada: e.target.value})} placeholder="Rua, Nº, Cidade" style={{ width: "100%", padding: "8px", borderRadius: 6, border: inputBorder, fontSize: 12, backgroundColor: inputBg, color: textColor }} /></div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 24, backgroundColor: bgColor, borderRadius: 16, padding: 24, border: cardBorder }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            <h3 style={{ fontFamily: isPublicView ? "Montserrat, sans-serif" : theme.fontFamily.sans, fontWeight: 800, fontSize: isPublicView ? 16 : 14, color: textColor }}>
              {isPublicView ? "🏷️ Marcas e Redes Sociais" : "Marcas e Redes Sociais"}
            </h3>
            {!isPublicView && <button onClick={adicionarMarca} style={{ padding: "6px 12px", borderRadius: 6, backgroundColor: theme.colors.accent.secondary, color: "#fff", border: "none", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>+ Adicionar</button>}
          </div>
          {marcas.map((marca, idx) => (
            <div key={marca.id} style={{ backgroundColor: isPublicView ? "#ffffff" : theme.colors.bg.primary, borderRadius: 10, padding: 12, marginBottom: 8, border: isPublicView ? "1px solid #e0ddd9" : `1px solid ${theme.colors.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 12, color: textColor }}>Marca {idx + 1}</span>
              ...
              <input type="text" value={marca.nome} onChange={e => setMarcas(prev => prev.map(m => m.id === marca.id ? { ...m, nome: e.target.value } : m))} placeholder="Nome da marca" style={{ width: "100%", padding: "6px 8px", borderRadius: 6, border: isPublicView ? "1px solid #ddd" : `1px solid ${theme.colors.border}`, fontSize: 11, marginBottom: 8, backgroundColor: inputBg, color: textColor }} />
      ...
      <div style={{ flex: isPublicView ? "0 1 320px" : "0 0 320px", position: "sticky", top: isPublicView ? 100 : 40 }}>
        <div style={{ backgroundColor: bgColor, borderRadius: 16, padding: 24, border: cardBorder, boxShadow: isPublicView ? "0 4px 20px rgba(0,0,0,0.08)" : "none" }}>
          ...
          <div style={{ backgroundColor: isPublicView ? "#ffffff" : theme.colors.bg.primary, borderRadius: 12, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px dashed ${theme.colors.border}` }}><span style={{ fontSize: 12, color: theme.colors.text.secondary }}>IVA (23%)</span><span style={{ fontSize: 12, fontWeight: 600, color: textColor }}>{ivaComDesconto.toFixed(2)} €</span></div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            {propostaId ? (
              <a href={`/p/${propostaId}`} target="_blank" rel="noopener noreferrer" style={{ flex: 1, padding: "12px", borderRadius: 10, fontWeight: 700, fontSize: 12, backgroundColor: theme.colors.accent.primary, color: "#ffffff", border: "none", textDecoration: "none", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center" }}>{isPublicView ? "👁️ Preview Proposta" : "Ver Proposta"}</a>
            ) : (
              <button disabled style={{ flex: 1, padding: "12px", borderRadius: 10, fontWeight: 700, fontSize: 12, backgroundColor: isPublicView ? "#666666" : "#cccccc", color: isPublicView ? "#aaaaaa" : "#ffffff", border: "none" }}>{isPublicView ? "👁️ Guardar primeiro" : "Guardar primeiro"}</button>
            )}
          </div>
          <button onClick={handleGuardarProposta} disabled={!podeGerarProposta} style={{ width: "100%", padding: "14px", borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: podeGerarProposta ? "pointer" : "not-allowed", backgroundColor: podeGerarProposta ? theme.colors.accent.secondary : "#cccccc", color: "#ffffff", border: "none" }}>{isPublicView ? "💾 Guardar Proposta" : "Guardar Proposta"}</button>
        </div>
      </div>
    </div>
  );
