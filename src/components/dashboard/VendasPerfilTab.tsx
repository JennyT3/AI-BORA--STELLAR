...
import { Instagram, Facebook, Linkedin, Twitter, Save, Camera, Sparkles } from "lucide-react";
...
export function VendasPerfilTab({ vendedor, profileData, editProfile, isMobile, onProfileChange, onSave, onToggleEdit, onOpenConvite }: VendasPerfilTabProps) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={onOpenConvite} style={{ padding: "10px 20px", borderRadius: 10, background: 'linear-gradient(135deg, #F25C05 0%, #F22283 100%)', color: "#fff", border: "none", fontWeight: 800, cursor: "pointer", fontSize: 13, display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 8px 16px rgba(242, 92, 5, 0.2)' }}>
            <Sparkles size={16} /> Convidar Colega
          </button>
          {!editProfile && (
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: 24, border: "1px solid #e8e8e8", textAlign: "center" }}>
          ...
          {editProfile && (
            <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
              <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: theme.colors.accent.primary, fontSize: 13, fontWeight: 600 }}>
                <Camera size={16} /> Alterar Foto (URL)
              </span>
              <input type="text" value={profileData.fotoPerfil} onChange={(e) => onProfileChange('fotoPerfil', e.target.value)} placeholder="Cole o URL da sua foto" style={{ padding: "8px 12px", borderRadius: 8, border: `2px solid ${theme.colors.border}`, fontSize: 12, textAlign: "center", width: "80%" }} />
            </div>
          )}
        </div>
