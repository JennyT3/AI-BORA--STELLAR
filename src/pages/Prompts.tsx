...
import { prompts as localPrompts, categories } from '../data/prompts';
import type { Prompt } from '../data/prompts';
import { PromptCard } from '../components/PromptCard';
import { Search } from 'lucide-react';

// ── Google Sheet público como fuente de datos ──────────────────────────────
...
function parseCSV(csv: string): Prompt[] {
  ...
  // lines[0] = cabecera: id,titulo,prompt,imagen_url,categoria,fecha,activo
  return lines.slice(1).map((line, i) => {
    // split respetando comas dentro de comillas
    const cols = line.match(/(".*?"|[^",\r\n]+|(?<=,)(?=,)|^(?=,)|(?<=,)$)/g) ?? [];
    const clean = (s?: string) => (s ?? '').replace(/^"|"$/g, '').trim();
    const activo = clean(cols[6]).toUpperCase();
    if (activo === 'FALSE') return null;
    return {
      id: `sheet-${clean(cols[0]) || i}`,
      title: clean(cols[1]),
      prompt: clean(cols[2]),
      image: clean(cols[3]),
      category: clean(cols[4]) || 'Tendencias',
      isPremium: false,
      tags: [clean(cols[4]).toLowerCase(), 'tendencia'].filter(Boolean),
    } as Prompt;
...
export function Prompts() {
  ...
  // ── Cargar datos del Sheet al montar ──────────────────────────────────────
  useEffect(() => {
    fetch(SHEET_URL)
      .then(r => r.text())
      .then(csv => setSheetPrompts(parseCSV(csv)))
      .catch(() => {}); // si falla, se usan solo los locales
  }, []);

  // Sheet primero, luego los hardcodeados
  const allPrompts = useMemo(() => [...sheetPrompts, ...localPrompts], [sheetPrompts]);
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <main className="pt-20">
        ...
        {/* Badge de prompts del Sheet */}
        {sheetPrompts.length > 0 && (
          <div className="text-center pt-4">
            <span style={{
              display: 'inline-block',
              backgroundColor: 'rgba(255,111,46,0.15)',
              ...
              ✦ {sheetPrompts.length} novos prompts em tendência
            </span>
          </div>
        )}
