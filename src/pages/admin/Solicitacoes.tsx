...
const STATUS_OPTIONS = [
  { value: "pendente", label: "Pendente", color: "#FEF3C7", text: "#D97706" },
  { value: "em-analise", label: "Em Análise", color: "#DBEAFE", text: "#1D4ED8" },
  { value: "contactado", label: "Contactado", color: "#E0E7FF", text: "#4338CA" },
  { value: "proposta-enviada", label: "Proposta Enviada", color: "#E8F5E9", text: "#10B981" },
  { value: "convertido", label: "Convertido", color: "#DCFCE7", text: "#16A34A" },
  { value: "descartado", label: "Descartado", color: "#FEE2E2", text: "#DC2626" },
];

const ORIGEN_OPTIONS = [
  { value: "all", label: "Todos" },
  { value: "Simulador", label: "Simulador" },
  { value: "Colabora Connosco", label: "Colaboração" },
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: theme.colors.text.secondary, fontSize: 14 }}>{solicitudes.filter(s => s.status === "pendente").length} pendentes · {filteredSolicitudes.length} total</p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 60, color: theme.colors.text.secondary, backgroundColor: "#ffffff", borderRadius: 16, border: `1px solid ${theme.colors.border}` }}>Nenhuma solicitação encontrada.</div>
      ) : (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: isMobile ? 12 : 24, flexDirection: isMobile ? "column" : "row" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {s.observacoes && (
                      ...
                      >
                        {isExpanded ? "Ocultar" : "Ver"} observações {isExpanded ? "▲" : "▼"}
                      </button>
                    )}
